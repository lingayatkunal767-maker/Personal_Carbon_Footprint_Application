      ...
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col sticky top-0 h-screen">
        <div className="p-6 flex items-center gap-2">
          ...
          <div>
            ...
            {isAdmin && <p className="text-xs text-purple-600 font-semibold -mt-0.5">Admin</p>}
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-20">
          <div className="relative w-96">
            ...
            <input type="text" placeholder="Search goals or activities..."
              className="w-full bg-gray-50 border-none rounded-lg py-2 pl-10 pr-4 outline-none text-sm" />
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 border-l pl-4">
              <div>
                ...
                {isAdmin && <p className="text-xs text-purple-600 font-semibold">Admin</p>}
              </div>
              <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}`}
                className="w-10 h-10 rounded-full bg-emerald-50 border border-emerald-100" alt="avatar" />
            </div>
          </div>
        </header>
